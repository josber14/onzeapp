...
import { confirmTotpSetup } from "@/lib/usdt-totp";
...
export async function POST(req: NextRequest) {
  ...
  if (!/^\d{6}$/.test(code)) {
    return NextResponse.json({ ok: false, error: "Ingresa un código de 6 dígitos" }, { status: 400 });
  }

  const result = await confirmTotpSetup(session.clientId, code);
